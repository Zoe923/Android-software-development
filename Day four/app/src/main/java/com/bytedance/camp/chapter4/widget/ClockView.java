...
public class ClockView extends View {
    ...
    private void drawUnit(Canvas canvas) {
        for (int i = 0; i < unitLinePositions.size(); i++) {
            ...
            float lengthX = linePosition.right-linePosition.left;
            float lengthY = linePosition.bottom-linePosition.top;
            float dx = lengthX/5;
            float dy = lengthY/5;
            float tmpX=0;
            float tmpY=0;
            ...
            for(j=0;j<=5;j++)
            {
                tmpX = linePosition.left+dx * j;
                tmpY = linePosition.top + dy * j;
                canvas.drawPoint(tmpX,tmpY,unitPaint);
            }
        }
    }
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        int hourDegree = 360/12 * hour + 360/12/60 * minute - 90;
        ...
        int minuteDegree = 360/60 * minute + 360/60/60 * second - 90;
        ...
        int secondDegree = 360/60 * second - 90;
    ...
    private void drawTimeNumbers(Canvas canvas) {
        ...
        String Time;
        int i;
        for(i=0;i<FULL_CIRCLE_DEGREE;i+=UNIT_DEGREE)
        {
            int timecenterX = (int) (centerX + 0.8 * radius * Math.sin(Math.toRadians(i)));
            int timecenterY = (int) (centerY - 0.8 * radius * Math.cos(Math.toRadians(i)));
            Paint.FontMetrics fontMetrics = numberPaint.getFontMetrics();
            float top = fontMetrics.top;
            float bottom = fontMetrics.bottom;
            int baselineY = (int) (timecenterY + (bottom-top)/2 - bottom);
            if(i%30==0)
            {
                Time = i/30 +"";
                if(i<300)
                ...
                if(i==0)
                    Time = "12";
                canvas.drawText(Time,timecenterX,baselineY,numberPaint);
            }
        }
    }
